...
class ModelEvaluation():
    ...
    def RMSE(self, output_file):
        ...
        val_rmse = round(
            metrics.mean_squared_error(self.val_predict['prediction'],
                                       self.val_predict.price_return,
                                       squared=False), 4)
        tra_rmse = round(
            metrics.mean_squared_error(self.tra_predict['prediction'],
                                       self.tra_predict.price_return,
                                       squared=False), 4)

        rmse_tb = pd.DataFrame({
            'dataset': ["train", "validation"],
            'rmse': [tra_rmse, val_rmse],
            'pred_mean': [
                self.tra_predict['prediction'].mean(),
                self.val_predict['prediction'].mean()
            ],
            'pred_std': [
                self.tra_predict['prediction'].std(),
    ...
    def corr(self, output_file):
        ...
        with pd.ExcelWriter('output/stock_mixed_model_evaluation.xlsx',
                            mode="a") as writer:
            cor_tb.to_excel(writer,
                            sheet_name='Cor_pred_ori_depend',
                            index=True)
    ...
    def residual(self, port):
        ...
        fig_val_resid_return = px.scatter(
            self.val_predict,
            x="price_return",
            y="resid",
            labels={"price_return": "Five days return"})
        fig_val_resid = px.histogram(self.val_predict, x="resid")

        fig_tra_resid_return = px.scatter(
            self.tra_predict,
            x="price_return",
            y="resid",
            labels={"price_return": "Five days return"})
        fig_tra_resid = px.histogram(self.tra_predict,
                                     x="resid",
                                     labels={"resid": "model residuals"})

        if self.model == "Mixed":
            app.layout = html.Div([
                html.H3(self.model + ' Model Evaluation',
                        style={'paddingRight': '30px'}),
                dcc.Tabs([
                    dcc.Tab(label='Training Residuals',
                            children=[
                                html.Div([
                                    dcc.Graph(id='tra_resid_return',
                                              figure=fig_tra_resid_return),
                                    dcc.Graph(id='tra_resid',
                                              figure=fig_tra_resid)
                                ])
                            ]),
                    dcc.Tab(label='Validation Residuals',
                            children=[
                                html.Div([
                                    dcc.Graph(id='val_resid_return',
                                              figure=fig_val_resid_return),
                                    dcc.Graph(id='val_resid',
                        ...
                        children=[
                            html.Div([
                                html.Img(
                                    ...
                                    src=
                                    '/assets/xgb_shap_feature_importance.png',
                                    style={
                                        'height': '50%',
                                        'width': '50%'
                                    })
