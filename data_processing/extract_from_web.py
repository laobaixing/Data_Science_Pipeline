...
def download_stock(endpoint, stock, td_consumer_key, period):
    full_url = endpoint.format(stock_ticker= stock,periodType='year',
                               period=period,frequencyType='daily',frequency=1)
    page = requests.get(url=full_url,
                    params={'apikey' : td_consumer_key})
    content = json.loads(page.content)

    df = pd.json_normalize(content, record_path =['candles'], 
                           meta =['symbol','empty'], meta_prefix='',
                           record_prefix='')
...
def get_TD_stock_info(stock, period, history_data_link, quote_link, td_consumer_key):
    
    df_history = download_stock(history_data_link,stock, td_consumer_key, period)
