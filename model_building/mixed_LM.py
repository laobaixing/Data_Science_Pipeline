...
class MixedLM():
    def mixed_lm(self, input_data_file):
        ...
        # Normalize RSI
        df_train['RSI_cen'] = df_train.groupby('symbol').RSI.transform(
            lambda x: x - x.mean())
        df_val['RSI_cen'] = df_val.groupby('symbol').RSI.transform(
            lambda x: x - x.mean())

        features = ['average_13_60_down', "RSI_cen"]

        df_train = df_train.dropna(subset=['price_return'] + features)
        df_val = df_val.dropna(subset=['price_return'] + features)

        df_train = df_train.loc[~df_train.symbol.
                                isin(["BB", "QS", "AI", "MAXR", "PLTR"])]
        df_val = df_val[~df_val.symbol.isin(["BB", "QS", "AI", "MAXR", "PLTR"]
                                            )]

        # This model is only on the slope
        formula = 'price_return ~' + "+".join(features)
        mlm_mod = sm.MixedLM.from_formula(formula=formula,
                                          groups='symbol',
        ...
        val_predict = pd.concat(
            [df_val[['datetime', 'price_return']], val_predict], axis=1)

        tra_predict = pd.DataFrame(
            {'prediction': mlm_result.predict(df_train)})
        tra_predict = pd.concat(
            [df_train[['datetime', 'price_return']], tra_predict], axis=1)
